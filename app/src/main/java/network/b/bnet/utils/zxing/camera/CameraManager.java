...
package network.b.bnet.utils.zxing.camera;
...
public final class CameraManager {
    ...
    public Rect getFramingRect() {
        ...
        if (framingRect == null) {
            ...
            int topOffset = (screenResolution.y - height) / 4 - 25;
    ...
    public Rect getFramingRect(int width, int height) {
        Point screenResolution = configManager.getScreenResolution();
        if (framingRect == null) {
            if (camera == null) {
                return null;
            }
            int offY = ((screenResolution.y - Utils.dip2px(BNetApplication.getInstance().getApplicationContext(), 66f)) - height) /
                    2 + Utils.dip2px(BNetApplication.getInstance().getApplicationContext(), 66f);
            framingRect = new Rect((screenResolution.x - width) / 2, offY, (screenResolution.x - width) / 2 + width,
                    offY + height);
            Log.d(TAG, "Calculated framing rect: " + framingRect);
        }
        return framingRect;
